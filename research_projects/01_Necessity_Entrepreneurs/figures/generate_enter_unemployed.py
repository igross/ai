"""Generate a submission-style flow diagram for 'Enter Period Unemployed'."""
...
ax.text(x_s, y2, "Unemployed/\nEntrepreneur", multialignment="left", **txt)

ax.text(x_a, y1, "Worker", **txt)
ax.text(x_a, y3, "Entrepreneur", **txt)

ax.text(x_b, y1, "Matched", **txt)
ax.text(x_b, y2, "Unmatched", **txt)
...
# Route into matching states to the left of the labels (avoid text overlap).
ax.annotate("", xy=(x_b - 0.30, y1), xytext=(x_a + 1.65, y1), arrowprops=arrow)
ax.annotate("", xy=(x_b - 0.30, y2), xytext=(x_a + 1.65, y1), arrowprops=arrow)

# Start outgoing arrows to the right of matching labels.
ax.annotate("", xy=(x_c - 0.25, y1), xytext=(x_b + 1.45, y1), arrowprops=arrow)
ax.annotate("", xy=(x_c - 0.25, y2), xytext=(x_b + 1.55, y2), arrowprops=arrow)
...
out_png = Path(__file__).with_name("Enter Period Unemployed_submission.png")
out_pdf = Path(__file__).with_name("Enter Period Unemployed_submission.pdf")
