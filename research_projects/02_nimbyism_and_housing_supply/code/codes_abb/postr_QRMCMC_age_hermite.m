function fval=postr_QRMCMC_age_hermite(Matdraw)
global Y N T Ntau Vectau Resqinit_eps Resqinit Resqinit_e0 b1_eps bL_eps b1 bL b1_e0 bL_e0 K1 K2 AGE MatAGE_t MatAGE1  meanAGE stdAGE meanY stdY
...
dens2=zeros(N,T);
for jtau=1:Ntau-1
    dens2(:,1)=dens2(:,1)+(Vectau(jtau+1)-Vectau(jtau))./((Resqinit_e0(:,jtau+1)-Resqinit_e0(:,jtau))'*MatAGE1')'.*...
        (Matdraw(:,1)>(Resqinit_e0(:,jtau)'*MatAGE1')').*(Matdraw(:,1)<=(Resqinit_e0(:,jtau+1)'*MatAGE1')');
end

dens2(:,1)=dens2(:,1)+Vectau(1)*b1_e0*exp(b1_e0*(Matdraw(:,1)-(Resqinit_e0(:,1)'*MatAGE1')')).*...
    (Matdraw(:,1)<=(Resqinit_e0(:,1)'*MatAGE1')')+...
    (1-Vectau(Ntau))*bL_e0*exp(-bL_e0*(Matdraw(:,1)-(Resqinit_e0(:,Ntau)'*MatAGE1')')).*...
    (Matdraw(:,1)>(Resqinit_e0(:,Ntau)'*MatAGE1')');
