...
drop if log_w == .		  /* drop if labor income missing */
...
* wage regression (males) 
reg log_w kidsout bigcity extra _I*
...
gen double uw2 = uw^2
reg uw2 `varlist2'
predict double uw2_yhat if e(sample)
gen muw = uw/sqrt(uw2_yhat)
...
*by year, sort: summarize wage totcons tot_assets3, det

log close

saveold first_stage_regsmalewages, replace 
sort person year
order uw uc ua age 
outsheet uw uc ua age using first_stage_regsmalewages, nonames replace
