...
% This code simulates impulse responses for income and consumption
% The results are obtained by varying tau_init and tau_shock lines 58-59
% acc=1 is nonlinear assets rule, acc=2 is linear assets rule
...
for jj=1:nage
    ...
    if jj<=nage-1
       ...
       if acc==1
                       ...
                       for mm4=0:R4
                           for mm5=0:R5
                               XXA=[XXA hermite(mm1,(A(:,jj)-meanA)/stdA).*hermite(mm2,(C(:,jj)-meanC)/stdC).*hermite(mm3,(Y(:,jj)-meanY)/stdY)...
                                   .*hermite(mm4,(Mateta_true(:,jj)-meanY)/stdY).*hermite(mm5,(aa-meanAGE)/stdAGE)];
                           end
                       end
                   end
               end
           ...
           A(:,jj+1)=XXA*Restrue_a2(1:(R1+1)*(R2+1)*(R3+1)*(R4+1)*(R5+1),1)+sqrt(Restrue_a2((R1+1)*(R2+1)*(R3+1)*(R4+1)*(R5+1)+1,1))*randn(N,1);
...
% modify axes depending of the values of "tau_shock" and "tau_init"

% figure
% plot((aa_ref:2:59)',[nanmedian(Y)'-nanmedian(Y1)'])
% %plot((aa_ref:2:63)',[nanmedian(Y)'-nanmedian(Y1)' quantile(Y,.25)'-quantile(Y1,.25)' quantile(Y,.75)'-quantile(Y1,.75)'])
% axis([35 65 0 .30])
% %axis([35 65 -.30 0])
% figure
% plot((aa_ref:2:59)',[nanmedian(C)'-nanmedian(C1)'])
% %plot((aa_ref:2:63)',[nanmedian(C)'-nanmedian(C1)' quantile(C,.25)'-quantile(C1,.25)' quantile(C,.75)'-quantile(C1,.75)'])
% axis([35 65 0 .15])
% %axis([35 65 -.15 0])
...
elseif tau_shock==.9
    % For Figure 8 when acc=2, for Figure S31 when acc=1.
    figure
    plot((aa_ref:2:59)',[nanmedian(Y)'-nanmedian(Y1)'],'-','Linewidth',3,'Color','b')
    xlabel('age','FontSize',20)
    set(gca,'xlim',[35 59])
