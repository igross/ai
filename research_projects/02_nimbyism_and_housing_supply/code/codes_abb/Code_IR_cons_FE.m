...
% This code produces the impulse responses in the model with unobserved heterogeneity in consumption. 
...
for mm7=0:M7
    for mm8=0:M8
        for mm9=0:M9
            Mata1eta1=[Mata1eta1 hermite(mm7,(A(:,1)-meanA)/stdA).*hermite(mm8,(Mateta_true(:,1)-meanY)/stdY).*hermite(mm9,(aa_ref-meanAGE)/stdAGE)];
        end
    end
end
...
%First quantile
Indiv=(Mata1eta1*Restrue_xi(:,1)).*(V_draw<=Vectau(1));
for jtau=2:Ntau
    Indiv=Indiv+((Mata1eta1*(Restrue_xi(:,jtau)-Restrue_xi(:,jtau-1)))/(Vectau(jtau)-Vectau(jtau-1)).*...
        (V_draw-Vectau(jtau-1))+Mata1eta1*Restrue_xi(:,jtau-1)).*(V_draw>Vectau(jtau-1)).*(V_draw<=Vectau(jtau));
end
%Last quantile.
Indiv=Indiv+(Mata1eta1*Restrue_xi(:,Ntau)).*(V_draw>Vectau(Ntau));

% Laplace tails
Indiv=Indiv+((1/(b1true_xi)*log(V_draw/Vectau(1))).*(V_draw<=Vectau(1))...
