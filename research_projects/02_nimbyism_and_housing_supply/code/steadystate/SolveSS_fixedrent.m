...
Ipen = logical( 0*(-bbb<=aaa*a_price*CC)+(1).*(-bbb>aaa*a_price*CC) );
...
for n=1:maxit
    ...
    Vbf(I,:,:) = (zzz(I,:,:) + Rb(I,:,:).*bmax-r_price.*rent(I,:,:) + aaa_annuity(I,:,:)).^(-s);
    Vbb(1,:,:) = (zzz(1,:,:) + Rb(1,:,:).*bmin-r_price.*rent(1,:,:) + aaa_annuity(1,:,:)).^(-s);
    Vbb(Ipen) = (zzz(Ipen) + Rb(Ipen).*bmin-r_price.*rent(Ipen) + aaa_annuity(I)).^(-s);
    
    totalincome=zzz + Rb.*bbb-r_price.*rent + aaa_annuity;
    ...
    sf = zzz + Rb.*bbb - cf - r_price.*rent +aaa_annuity;
    
    %consumption and savings with backward difference
    sb = zzz + Rb.*bbb - cb - r_price.*rent +aaa_annuity;
    ...
    c0 = zzz + Rb.*bbb - r_price.*(rent) + aaa_annuity;
    ...
    u = c.^(1-s)/(1-s)+theta_h*(h).^(1-s_h)/(1-s_h)-1e4.*(Ipen);%
    ...
    ss = zzz + Rb.*bbb - c - rent*r_price + aaa_annuity;
...
%     prev_rent=rent;
%     optimal_rent(:,1,:) = min((Vb_Upwind(:,1,:)/theta_r/theta_h).^(-1/s),amax);
%
%     if 1
%
%     if nr==2
