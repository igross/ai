function [distance] = SolveSS_function(x)
...
s_h = 0.5;%x(3);% Weight on houing in utility
theta_r = 0.85;%x(4);% Discount on rent utility
beta=0.98; %Discount rate

ka=0.06; %Fraction of house price in adjustment costs
bequestweight=0.9*beta; % Relative weight of bequests
            ...
            for iz=1:K
                for ij=1:J
                    for ii=1:I
                        ...
                        consumption_plane(:,1,iz,ilm) = max( ( exp(z(iz))*Zlifecycle(age_n-iage+1)   + (Rb_line(ii)).*b(ii) + a(ij)*a_price - aaaa(:,1,iz,ilm)*a_price.*(1+ka*(1-(aaaa(:,1,iz,ilm)==a(ij)))) + b(ii) - bb(:,1) )/(1+s_h*theta_r),1e-20);
                        ...
                        if Ipricepreferences
                            ...
                            consumption_plane(:,1,iz,ilm) = max( ( exp(z(iz))*Zlifecycle(age_n-iage+1)   + (Rb_line(ii)).*b(ii) + a(ij)*(a_price*d_a_price) - aaaa(:,1,iz,ilm)*a_price*d_a_price.*(1+ka*(1-(aaaa(:,1,iz,ilm)==a(ij)))) + b(ii) - bb(:,1) )/(1+s_h*theta_r),1e-20);
            ...
            for iz=1:K
                ...
                for ij=1:J
                    for ii=1:I
                        ...
                        consumption_plane(:,2:J,iz,ilm) = max( exp(z(iz))*Zlifecycle(age_n-iage+1)  + (Rb_line(ii)).*b(ii) + a(ij)*a_price - aaaa(:,2:J,iz,ilm)*a_price.*(1+ka*(1-(aaaa(:,2:J,iz,ilm)==a(ij)))) + b(ii) - bbbb(:,2:J,iz,ilm),1e-20);
                        consumption_plane(:,1,iz,ilm) = max( ( exp(z(iz))*Zlifecycle(age_n-iage+1)   + (Rb_line(ii)).*b(ii)  + a(ij)*a_price - aaaa(:,1,iz,ilm)*a_price.*(1+ka*(1-(aaaa(:,1,iz,ilm)==a(ij)))) + b(ii) - bbbb(:,1,iz,ilm) )/(1+s_h*theta_r),1e-20);
                        rent_plane(:,1,iz,ilm) = consumption_plane(:,1,iz,ilm)./r_price*theta_r*s_h;
                        ...
                        if Ipricepreferences
                            ...
                            consumption_plane(:,1,iz,ilm) = max( ( exp(z(iz))*Zlifecycle(age_n-iage+1)   + (Rb_line(ii)).*b(ii)  + a(ij)*a_price*d_a_price - aaaa(:,1,iz,ilm)*a_price*d_a_price.*(1+ka*(1-(aaaa(:,1,iz,ilm)==a(ij)))) + b(ii) - bbbb(:,1,iz,ilm) )/(1+s_h*theta_r),1e-20);
                            rent_plane(:,1,iz,ilm) = consumption_plane(:,1,iz,ilm)./(r_price*d_a_price)*theta_r*s_h;
...
cohortsize= [  10	   10	   10	   10	   10	   10	   10	   9	   7	   5	   4	   3	   2	   1];
...
distance = 100*sum(dens4.*pref4,'all')^2 + sum(bbbb.*dens4,'all')^2 ;
save SS_function_boomer
end