...
lygrid =  log(ygrid);
midlygrid = (lygrid(2:end)+lygrid(1:end-1))/2;

edgelygrid=[-100,midlygrid,100];

y_vector=[-1e2,y_vector,1e2];
...
initialdist=sum((y_discrete(:,1)==[1:6]),1) ;
initialdist=initialdist./sum(initialdist);

%Count up up the transitions between discretised
%points.
transitioncount=ones(gridsize,gridsize,size(age_disvec,2)-1);
...
save TransitionMatrix transitionmatrix  transitionmatrix_all initialdist
